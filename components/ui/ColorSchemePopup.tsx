...
const ColorSchemePopup: React.FC<ColorSchemePopupProps> = ({ visible, onClose, onSelect }) => {
  ...
  const borderColor = themes.theme === "dark" ? "#404040" : "rgb(210, 210, 210)";
  const borderPressedColor =
    themes.theme === "dark" ? "rgba(255, 255, 255, 0.7)" : "rgba(80, 80, 80, 0.7)";
  const backgroundPressedColor =
    themes.theme === "dark" ? "rgba(255, 255, 255, 0.1)" : "rgba(80, 80, 80, 0.1)";
  ...
  useEffect(() => {
    ...
    let backHandler: ReturnType<typeof BackHandler.addEventListener> | undefined;
    if (visible) {
  ...
  const RadioButton: React.FC<RadioButtonProps> = ({ label, value, isSelected, onPress }) => {
    const [isButtonPressed, setIsButtonPressed] = useState(false);
    const scaleAnim = useRef(new Animated.Value(1)).current;
    const innerScaleAnim = useRef(new Animated.Value(isSelected ? 1 : 0)).current;
    ...
    <TouchableWithoutFeedback onPress={closePopup}>
      ...
      >
        <TouchableWithoutFeedback onPress={(e) => e.stopPropagation()}>
          ...
          >
            <Text style={[styles.title, { color: textColor, borderBottomColor: borderColor }]}>
              Color Scheme
            </Text>
